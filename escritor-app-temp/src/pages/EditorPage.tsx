...
import { generateWithAI, AIRequest } from '../components/ClaudeAIService';
...
export default function EditorPage() {
  ...
  const { 
    ...
    error: chaptersError
  } = useChapters(bookId ? parseInt(bookId) : undefined);
  ...
  // Set active book ID from the URL
  useEffect(() => {
    if (bookId && !livrosLoading) {
      const numericBookId = parseInt(bookId);
      setActiveLivroId(numericBookId);
      
      // Check if the book exists and belongs to the user
      const bookExists = livros.some(book => book.id === numericBookId);
      if (!bookExists) {
        navigate('/dashboard');
      }
    }
  }, [bookId, livros, livrosLoading, setActiveLivroId, navigate]);

  // Set active chapter ID from the URL or default to the first chapter
  useEffect(() => {
    if (chapterId && !chaptersLoading) {
      const numericChapterId = parseInt(chapterId);
      setActiveChapterId(numericChapterId);
      
      // Check if the chapter exists in the book
      const chapterExists = chapters.some(chapter => chapter.id === numericChapterId);
      if (!chapterExists) {
  ...
  }, [chapterId, chapters, chaptersLoading, bookId, activeChapterId, setActiveChapterId, navigate]);

  // Update text when active chapter changes
  useEffect(() => {
    if (activeChapter) {
      setText(activeChapter.texto || '');
    }
  }, [activeChapterId, activeChapter]);

  // Update the URL when active chapter changes
  useEffect(() => {
    if (bookId && activeChapterId && !chapterId) {
      navigate(`/editor/${bookId}/${activeChapterId}`, { replace: true });
    }
  }, [bookId, activeChapterId, chapterId, navigate]);
  ...
  useEffect(() => {
    if (editingChapterTitle !== null && titleInputRef.current) {
  ...
  const handleEditChapterTitle = (id: number, currentTitle: string | null) => {
    setEditingChapterTitle(id);
  ...
  const handleSaveChapterTitle = async () => {
    if (editingChapterTitle !== null) {
      await updateChapterTitle(editingChapterTitle, newChapterTitle);
    ...
    try {
      const request: AIRequest = {
        ...
        context: {
          ...
          livro_id: activeLivroId
        }
      };
      
      const response = await generateWithAI(request);
      setAiResult(response.result || null);
      setShowAIPanel(true);
    } catch (error) {
  ...
  if (authLoading || livrosLoading || chaptersLoading) {
    return (
        ...
        <aside className="sidebar">
            ...
            {chapters.map(chapter => (
              ...
              >
                {editingChapterTitle === chapter.id ? (
                  <div className="chapter-title-edit">
                    <input
                      ref={titleInputRef}
                      type="text"
                  ...
                  <div 
                    ...
                    onDoubleClick={() => handleEditChapterTitle(chapter.id, chapter.titulo)}
                  >
                    {chapter.titulo || `Capítulo ${chapter.id}`}
                  </div>
                )}
                
                {chapter.created_at && (
                  <div className="chapter-meta">
                    Última atualização: {new Date(chapter.created_at).toLocaleDateString()}
                  </div>
                )}
                ...
                <div className="chapter-actions">
                  <button 
                    ...
                    onClick={() => handleEditChapterTitle(chapter.id, chapter.titulo)}
          ...
          <div className="book-info-panel">
            <h3 className="book-info-title">
              {activeLivro?.["Nome do livro"] || 'Livro sem título'}
            </h3>
            <div className="book-info-stats">
              <div className="book-info-stat">
                <span className="book-info-stat-label">Capítulos:</span>
                <span className="book-info-stat-value">{chapters.length}</span>
              </div>
              <div className="book-info-stat">
                <span className="book-info-stat-label">Palavras:</span>
                <span className="book-info-stat-value">{getTotalWordCount()}</span>
              </div>
            </div>
          </div>
        </aside>
            ...
            <div className="content-panel">
              ...
              <div className="progress-section">
                ...
                <div className="progress-list">
                  {chapters.map(chapter => {
                    const words = chapter.texto ? chapter.texto.trim().split(/\s+/).filter(word => word.length > 0).length : 0;
                    ...
                    return (
                      <div key={chapter.id} className="progress-item">
                        <div className="progress-header">
                          <span>{chapter.titulo || `Capítulo ${chapter.id}`}</span>
